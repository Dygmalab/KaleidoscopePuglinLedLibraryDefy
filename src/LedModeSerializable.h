...
template<unsigned int CRC, char... Chars>
struct Crc32Impl {
};

template<unsigned int CRC, char Head, char... Tail>
struct Crc32Impl<CRC, Head, Tail...> {
  static constexpr unsigned int value = Crc32Impl<
    crc32_table[static_cast<unsigned char>(CRC) ^ static_cast<unsigned char>(Head)] ^ (CRC >> 8),
    Tail...>::value;
};
...
constexpr unsigned int CRC32(const char *s, size_t len) {
  return crc32_rec(0xFFFFFFFF, s);
}
